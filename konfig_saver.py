...
try:
    for t in range(len(Konfig_json["device_to_config"])):
        try:
            ...
            print("      Starting...")
            try:
                for config in range(len(Konfig_json["device_to_config"][t]["config"])):
                    output = net_connect.send_config_set(Konfig_json["device_to_config"][t]["config"][config])
                    print("      ended...")
            except EXCEPTION as e:
                print("      Can't send config to a file")
        except:
